...
from DeepHedging.Agents import SimpleAgent, RecurrentAgent, LSTMAgent, GRUAgent, WaveNetAgent
...
def get_agent(agent_name, path_transformation_configs, n_hedging_timesteps):
    if agent_name not in TRAINABLE_AGENTS:
        raise ValueError(
...
def get_contingent_claim(claim_type, strike):
    ...
    if claim_type not in claims:
        raise ValueError(
            f"Contingent Claim '{claim_type}' is not recognized. Available claims: {list(claims.keys())}"
        )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--agent",
        type=str,
        default="RecurrentAgent",
        choices=list(TRAINABLE_AGENTS.keys()),
        help="Type of trainable agent (default: RecurrentAgent)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="optimizers",
        help="Directory to save optimizers (default: optimizers)",
    )
...
def main():
    ...
    # Cost function and risk measure
